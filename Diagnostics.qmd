...
The following diagnostic horizons and soil characteristics have been modified from Soil Taxonomy [@soil_survey_staff_soil_1999]: [argillic horizon](#sec-diag-argh), [duripan](#sec-diag-dpan), [fragipan](#sec-diag-fpan), [lithic contact](#sec-diag-lithc), [paralithic contact](#sec-diag-plith) and [placic horizon](#sec-diag-plac).
...
The nodules are common features in [Oxidic Soils](key-to-orders.qmd#sec-ord-X) and some [Granular Soils](key-to-orders.qmd#sec-ord-N) [@wilson_classification_1987]. The frequency distribution of nodules is clustered in the \< 2% range and the \> 15% range. Few profiles are known to lie in between.
...
Organic soil materials that have been accumulated under wet conditions are subdivided into three classes, based on evidence of decomposition [@clayden_horizon_1989]. These classes are used to distinguish soil groups of [Organic Soils](key-to-orders.qmd#sec-ord-O).
