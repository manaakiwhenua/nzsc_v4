...
The name given to a soil assigned to a subgroup is made up of three elements in the sequence: subgroup, group, and order (for example, [Nodular Perch-gley Oxidic Soils](#key-XPN)). Figure 1 illustrates the relationships between subgroups and groups in the [Oxidic Soils](#ord-X) order.

```{r,echo=FALSE,fig.show='hold', fig.cap='The hierarchy of the [Oxidic Soils](#ord-X) as an example of the hierarchical relationships between orders, groups and subgroups. As the diagram suggests, the range of soil properties for each class is related to hierarchical position',out.width = '85%',fig.align='center'}
